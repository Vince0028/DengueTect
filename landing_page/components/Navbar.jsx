...
const Navbar = () => {
    ...
    return (<>
      <motion.nav initial={{ y: -100 }} animate={{ y: 0 }} transition={{ duration: 0.5 }} className={`fixed top-0 left-0 right-0 z-50 flex justify-center transition-all duration-300 ${scrolled ? 'pt-4' : 'pt-6'}`}>
        <div className={`
          relative flex items-center justify-between px-6 transition-all duration-300
          ${scrolled
            ? 'w-[90%] max-w-6xl bg-dengue-dark/80 backdrop-blur-xl border border-dengue-primary/20 rounded-full py-3 shadow-[0_8px_32px_rgba(22,163,74,0.15)]'
            : 'w-full container mx-auto py-2 bg-transparent border-transparent'}
        `}>
          ...
          <div className="hidden md:flex items-center gap-1 bg-dengue-light/30 rounded-full px-2 py-1 border border-white/5">
            {['Features', 'How it Works', 'Pricing', 'Education'].map((item) => (<a key={item} href={`#${item.toLowerCase().replace(/\s+/g, '-')}`} className="px-4 py-1.5 text-sm font-medium text-slate-300 hover:text-white hover:bg-white/5 rounded-full transition-all">
                {item}
              </a>))}
          </div>
      ...
      <AnimatePresence>
        {isOpen && (<motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="fixed inset-0 top-20 z-40 bg-dengue-dark p-6 md:hidden">
            <div className="flex flex-col gap-6 text-center">
              {['Features', 'How it Works', 'Pricing', 'Education'].map((item) => (<a key={item} href="#" className="text-2xl font-display font-bold text-slate-300 hover:text-dengue-glow" onClick={() => setIsOpen(false)}>
                  {item}
                </a>))}
