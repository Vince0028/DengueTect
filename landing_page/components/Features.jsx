...
const Features = () => {
    ...
    return (<section id="features" className="py-20 md:py-32 relative overflow-hidden" ref={ref}>
      <div className="container mx-auto px-6 relative z-10">
        
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 md:mb-20 gap-6">
            <div className="max-w-2xl">
                <motion.h2 initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="text-3xl md:text-6xl font-display font-bold mb-4 md:mb-6 text-white">
                    Diagnostic <br />
                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-dengue-primary to-dengue-glow">Ecosystem</span>
                </motion.h2>
                <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.2 }} className="text-slate-400 text-base md:text-lg max-w-md">
          ...
          <motion.div style={{ rotateX: tilt }} className="md:col-span-8 glass-card-strong rounded-[2rem] md:rounded-[2.5rem] p-8 md:p-14 overflow-hidden relative group min-h-[400px] md:min-h-[500px] flex flex-col justify-between preserve-3d transition-all duration-300">
            ...
            <div className="relative z-10 max-w-xl" style={{ transform: "translateZ(30px)" }}>
              ...
              <div className="flex flex-wrap gap-3">
                {['Photo Upload', 'Visual Reference', 'Bite Tracking'].map((item, i) => (<span key={i} className="px-3 py-1.5 md:px-4 md:py-2 rounded-full bg-white/5 border border-white/5 text-xs md:text-sm text-slate-300">
                    {item}
                  </span>))}
              </div>
            </div>
