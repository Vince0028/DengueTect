...
const InteractiveScanner = () => {
    ...
    return (<section ref={ref} className="py-20 md:py-32 relative overflow-hidden bg-black/20 perspective-2000">
      
      <div className="container mx-auto px-6 relative z-10 flex flex-col items-center">
        <div className="text-center mb-12 md:mb-20">
             <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-3xl md:text-5xl font-display font-bold text-white mb-4">
              See it in <span className="text-dengue-glow">Action</span>
            </motion.h2>
            <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="text-slate-400 max-w-xl mx-auto text-sm md:text-base">
        ...
        <motion.div className="relative w-[280px] h-[560px] md:w-[320px] md:h-[640px] preserve-3d" initial={{ rotateY: 0 }} whileInView={{ rotateY: [0, -10, 10, 0] }} transition={{ duration: 10, repeat: Infinity, ease: "easeInOut" }}>
                ...
                <div className="relative h-full w-full bg-dengue-dark flex flex-col font-sans">
                    ...
                    <div className="relative flex-1 bg-[url('https://images.unsplash.com/photo-1550666610-27c225d613b2?auto=format&fit=crop&q=80')] bg-cover bg-center opacity-90">
                        ...
                        {!scanComplete && (<>
                                ...
                                <div className="absolute top-1/4 left-8 bg-black/60 backdrop-blur text-white text-[10px] px-2 py-1 rounded border border-white/10 animate-pulse">
                                    Analyzing Tissue...
                                </div>
                            </>)}
                    </div>
            ...
            <motion.div initial={{ z: 0, y: 100, opacity: 0, scale: 0.8 }} animate={scanComplete ? { z: 60, y: -140, opacity: 1, scale: 1 } : { z: 0, y: 100, opacity: 0, scale: 0.8 }} transition={{ type: "spring", damping: 20, stiffness: 100 }} className="absolute bottom-8 left-4 right-4 bg-dengue-dark/95 backdrop-blur-xl rounded-2xl md:rounded-3xl border border-dengue-glow/30 p-5 md:p-6 shadow-[0_20px_50px_rgba(0,0,0,0.5)] flex flex-col">
                 ...
                 <button className="w-full mt-4 py-2.5 md:py-3 bg-dengue-primary hover:bg-dengue-glow text-dengue-dark font-bold rounded-xl transition-colors text-xs md:text-sm">
                    Full Report
                 </button>
            </motion.div>
