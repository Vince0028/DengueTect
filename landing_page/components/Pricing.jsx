...
const Pricing = () => {
    return (<section id="pricing" className="py-20 md:py-24 relative">
      <div className="container mx-auto px-6">
        <div className="text-center max-w-3xl mx-auto mb-12 md:mb-16">
          <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-3xl md:text-4xl font-display font-bold mb-4 text-white">
            Accessible Protection
          </motion.h2>
          <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.2 }} className="text-slate-400 text-base md:text-lg">
        ...
        <div className="grid md:grid-cols-3 gap-6 md:gap-8 max-w-6xl mx-auto items-stretch">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="glass-card rounded-3xl p-8 flex flex-col h-auto md:h-[420px]">
            ...
            <ul className="space-y-4 mb-8 flex-1">
              {['Unlimited Bite Uploads', 'Symptom Checker', 'Nearby Hospital Finder', 'Dengue Information'].map((feat) => (<li key={feat} className="flex items-center gap-3 text-sm text-slate-300">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="relative bg-gradient-to-b from-dengue-dark to-[#062c22] border border-dengue-primary/50 rounded-3xl p-8 flex flex-col h-auto md:h-[500px] shadow-[0_0_50px_-12px_rgba(22,163,74,0.4)] z-10">
            ...
            <ul className="space-y-4 mb-8 flex-1">
              {['Multiple Profiles', 'Family Dashboard', 'Shared Reports', 'Priority Support'].map((feat) => (<li key={feat} className="flex items-center gap-3 text-sm text-white">
                  <Check className="w-4 h-4 text-dengue-glow"/> {feat}
                </li>))}
            </ul>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.3 }} className="glass-card rounded-3xl p-8 flex flex-col h-auto md:h-[420px]">
            ...
            <ul className="space-y-4 mb-8 flex-1">
              {['Area Reports', 'Bulk Monitoring', 'Custom Dashboard'].map((feat) => (<li key={feat} className="flex items-center gap-3 text-sm text-slate-300">
