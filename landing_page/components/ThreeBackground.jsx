...
function DNADoubleHelix({ position, scale = 1, rotation }) {
    ...
    return (<group ref={groupRef} position={position} scale={scale}>
      {/* Backbone strands */}
      {helixData.points.map((point, i) => (<mesh key={`strand-${i}`} position={point.position}>
          <sphereGeometry args={[0.06, 8, 8]}/>
          <meshStandardMaterial color={point.strand === 1 ? '#22c55e' : '#16a34a'} transparent opacity={0.8} emissive={point.strand === 1 ? '#22c55e' : '#16a34a'} emissiveIntensity={0.3}/>
        </mesh>))}
      ...
      {helixData.basePairs.map((pair, i) => {
            ...
            const length = Math.sqrt(Math.pow(pair.end[0] - pair.start[0], 2) +
                Math.pow(pair.end[2] - pair.start[2], 2));
            const angle = Math.atan2(pair.end[2] - pair.start[2], pair.end[0] - pair.start[0]);
            return (<mesh key={`pair-${i}`} position={midPoint} rotation={[0, -angle, Math.PI / 2]}>
            <cylinderGeometry args={[0.03, 0.03, length, 8]}/>
            <meshStandardMaterial color={pair.color} transparent opacity={0.6} emissive={pair.color} emissiveIntensity={0.2}/>
          </mesh>);
        })}
    </group>);
}
// Floating Bacteria - rod-shaped with flagella
function Bacteria({ position, scale = 1, color = '#22c55e' }) {
    const groupRef = useRef(null);
    ...
    return (<group ref={groupRef} position={position} scale={scale}>
      ...
      <mesh>
        ...
        <MeshDistortMaterial color={color} distort={0.1} speed={2} roughness={0.3} metalness={0.2} transparent opacity={0.7} emissive={color} emissiveIntensity={0.2}/>
      </mesh>
      
      {/* Flagella (tail-like structures) */}
      {[0, 1, 2].map((i) => {
            const angle = (i / 3) * Math.PI * 0.5 - Math.PI * 0.25;
            return (<mesh key={i} position={[-0.35, Math.sin(angle) * 0.1, Math.cos(angle) * 0.1]} rotation={[angle, 0, Math.PI / 2 + 0.3]}>
            <torusGeometry args={[0.15, 0.02, 8, 16, Math.PI]}/>
            <meshStandardMaterial color={color} transparent opacity={0.5} emissive={color} emissiveIntensity={0.1}/>
          </mesh>);
        })}
    </group>);
}
// Spherical Bacteria (Cocci)
function SphericalBacteria({ position, scale = 1, color = '#16a34a' }) {
    ...
    return (<mesh ref={meshRef} position={position} scale={scale}>
      <sphereGeometry args={[0.2, 16, 16]}/>
      <MeshDistortMaterial color={color} distort={0.2} speed={3} roughness={0.4} metalness={0.1} transparent opacity={0.6} emissive={color} emissiveIntensity={0.2}/>
    </mesh>);
}
// Virus-like particle with spikes
function VirusParticle({ position, scale = 1, color = '#22c55e' }) {
    const groupRef = useRef(null);
    ...
    return (<group ref={groupRef} position={position} scale={scale}>
      ...
      <mesh>
        ...
        <MeshDistortMaterial color={color} distort={0.15} speed={2} roughness={0.3} metalness={0.3} transparent opacity={0.7} emissive={color} emissiveIntensity={0.3}/>
      </mesh>
      
      {/* Spikes */}
      {spikes.map((spike, i) => (<mesh key={i} position={spike.position}>
...
function DNAHelix({ position, color }) {
    const meshRef = useRef(null);
    useFrame((state) => {
        if (meshRef.current) {
    ...
    return (<Float speed={2} rotationIntensity={0.5} floatIntensity={1}>
      <mesh ref={meshRef} position={position}>
        ...
        <MeshDistortMaterial color={color} attach="material" distort={0.3} speed={2} roughness={0.2} metalness={0.8} transparent opacity={0.6}/>
      </mesh>
    </Float>);
}
function FloatingParticles() {
    ...
    return (<group ref={particlesRef}>
      {particles.map((particle, i) => (<mesh key={i} position={particle.position}>
          ...
          <meshStandardMaterial color={i % 2 === 0 ? "#22c55e" : "#16a34a"} transparent opacity={0.4} emissive={i % 2 === 0 ? "#22c55e" : "#16a34a"} emissiveIntensity={0.3}/>
        </mesh>))}
    </group>);
}
function GlowingOrb({ position, color, size = 1 }) {
    ...
    return (<Float speed={1.5} rotationIntensity={0} floatIntensity={2}>
      <mesh ref={meshRef} position={position}>
        <sphereGeometry args={[1, 32, 32]}/>
        <MeshDistortMaterial color={color} attach="material" distort={0.4} speed={3} roughness={0} metalness={0.1} transparent opacity={0.3}/>
      </mesh>
    </Float>);
}
// Random elements generator
function RandomDNAHelixes({ count = 5 }) {
    ...
    return (<>
      {helixes.map((helix, i) => (<DNADoubleHelix key={`dna-${i}`} position={helix.position} scale={helix.scale} rotation={helix.rotation}/>))}
    </>);
}
function RandomDNAKnots({ count = 3 }) {
    const colors = ['#22c55e', '#16a34a', '#4ade80'];
    const knots = useMemo(() => {
        return Array.from({ length: count }, () => ({
            position: [
    ...
    return (<>
      {knots.map((knot, i) => (<DNAHelix key={`knot-${i}`} position={knot.position} color={knot.color}/>))}
    </>);
}
function RandomBacteria({ count = 8 }) {
    const colors = ['#22c55e', '#16a34a', '#4ade80'];
    ...
    return (<>
      {bacteria.map((b, i) => (<Bacteria key={`bac-${i}`} position={b.position} scale={b.scale} color={b.color}/>))}
    </>);
}
function RandomSphericalBacteria({ count = 6 }) {
    const colors = ['#22c55e', '#16a34a', '#4ade80'];
    const bacteria = useMemo(() => {
        return Array.from({ length: count }, () => ({
            position: [
    ...
    return (<>
      {bacteria.map((b, i) => (<SphericalBacteria key={`sph-${i}`} position={b.position} scale={b.scale} color={b.color}/>))}
    </>);
}
function RandomVirusParticles({ count = 5 }) {
    const colors = ['#22c55e', '#16a34a', '#4ade80'];
    const particles = useMemo(() => {
        return Array.from({ length: count }, () => ({
            position: [
    ...
    return (<>
      {particles.map((p, i) => (<VirusParticle key={`vir-${i}`} position={p.position} scale={p.scale} color={p.color}/>))}
    </>);
}
// Main 3D Background component
const ThreeBackground = () => {
    return (<div className="fixed inset-0 z-0 pointer-events-none">
      <Canvas camera={{ position: [0, 0, 15], fov: 75 }} style={{ background: 'transparent' }} gl={{ alpha: true, antialias: true }}>
