...
export default function ReportBitePage() {
    ...
    function analyzeImageData(img, roiBox) {
        ...
        if (roiBox) {
            ...
            rx0 = Math.max(0, cx - rc);
            rx1 = Math.min(w - 1, cx + rc);
            ry0 = Math.max(0, cy - rc);
            ry1 = Math.min(h - 1, cy + rc);
        }
        for (let y = 0; y < h; y++) {
            for (let x = 0; x < w; x++) {
                const idx = (y * w + x) * 4;
    ...
    function decideLabel(stats) {
        ...
        if (!total)
            return { text: "No signal", cls: "muted" };
      ...
      <main className="container mx-auto px-4 py-6 max-w-md">
        <Card>
          ...
          <CardContent className="space-y-4">
            {!selectedImage ? (<div className="border-2 border-dashed border-border rounded-lg p-8 text-center">
                ...
                <div className="space-y-2">
                  ...
                  <input id="upload-input" type="file" accept="image/*" onChange={handleImageUpload} className="hidden"/>
                </div>
              </div>) : (<div className="space-y-4">
                <div className="relative">
                  <img ref={imgRef} src={selectedImage || "/placeholder.svg"} alt="Uploaded bite" className="w-full h-64 object-cover rounded-lg select-none" onLoad={() => analyzeNow()} onClick={onImageClick}/>
                  
                  {roi && (<div className="pointer-events-none absolute border-2 border-red-400/80 rounded-full shadow" style={{
                  ...
                  <div className="absolute bottom-2 right-2 flex flex-col items-end gap-1">
                    {indicator && (<div className={`text-xs px-2 py-1 rounded-md shadow ${indicator.cls === "red" ? "bg-red-600 text-white" : indicator.cls === "yellow" ? "bg-yellow-500 text-black" : "bg-black/60 text-white"}`}>
                        {indicator.text}
                      </div>)}
                    {debugStats && (<div className="text-[10px] leading-tight px-2 py-1 rounded-md bg-black/50 text-white shadow">
